...
bool PacmanEmulator::LoadRomSet(const std::string &romDir)
{
    ...
    size_t bytesRead = m_memory->LoadRom(romDir + "/" + graphicsPaletteFile, MemoryBus::ROMType::GRAPHICS_TILES, 0x0100);

    if (bytesRead != 0x0100) {
        std::cerr << "Failed: " << graphicsPaletteFile << " (read " << bytesRead << " bytes, expected 256)\n";
        return false;
    }
